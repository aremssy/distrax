...
@section('title', __('Reset Password | Rentdo'))

@section('content')
    <x-website-auth-shell
        eyebrow="{{ __('A simpler way to find home') }}"
        heading="{{ __('Choose a new password.') }}"
        description="{{ __('Pick something strong you\'ll remember. You\'ll be signed in and ready to go in seconds.') }}"
    >
        <div>
            <p class="text-sm font-semibold text-indigo-600">{{ __('Password recovery') }}</p>
            <h2 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">{{ __('Reset your password') }}</h2>
            <p class="mt-3 text-sm leading-6 text-slate-500">{{ __('Enter and confirm your new password below.') }}</p>
        </div>
        ...
        <form action="{{ route('password.update') }}" class="mt-7 grid gap-5" method="POST">
            @csrf
            ...
            <div class="grid gap-2">
                ...
                <div class="relative">
                    ...
                    <input
                        ...
                        id="email" name="email" type="email" value="{{ old('email', $email) }}" placeholder="you@example.com"
            ...
            <div class="grid gap-2">
                ...
                <div class="relative">
                    ...
                    <input
                        ...
                        id="password" name="password" type="password" placeholder="{{ __('At least 4 characters') }}"
                        autocomplete="new-password" required minlength="4" data-password-input
                    >
            ...
            <div class="grid gap-2">
                ...
                <div class="relative">
                    ...
                    <input
                        class="h-13 w-full rounded-xl border border-slate-200 bg-white py-3 pl-12 pr-12 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 hover:border-slate-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10"
                        id="password_confirmation" name="password_confirmation" type="password" placeholder="{{ __('Re-enter your new password') }}"
                        autocomplete="new-password" required minlength="4" data-password-input
                    >
                    <button class="absolute right-3 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-slate-700" type="button" data-password-toggle aria-label="{{ __('Show password') }}" aria-pressed="false">
                        <i class="h-4 w-4" data-lucide="eye"></i>

<x-admin-layout title="Blog Categories">
    <x-admin-page-header title="Blog Categories"
        :breadcrumbs="[['label' => 'Blog', 'route' => 'admin.blogs.index'], ['label' => 'Categories']]"
        description="Manage blog post categories.">
        <x-slot name="actions">
            <x-admin.export-button resource="blog-categories" />
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Posts</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800" data-sortable data-sortable-url="{{ route('admin.blogs.categories.reorder') }}" data-sortable-offset="{{ ($categories->currentPage() - 1) * $categories->perPage() }}">
                    @forelse ($categories as $category)
                        <tr class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40" data-id="{{ $category->id }}">
                            ...
                            <td class="min-w-60 max-w-72 px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    <div class="flex h-11 w-14 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-400 dark:bg-night-800 dark:text-night-500">
                                        @include('admin.partials.icon', ['name' => 'newspaper', 'class' => 'w-5 h-5'])
                                    </div>
                                    <div class="min-w-0">
                            ...
                            <td class="whitespace-nowrap px-5 py-3.5 text-right">
                                <div class="relative inline-block" x-data="kebabMenu()" @click.away="open = false">
                                    ...
                                    <div x-cloak x-show="open" x-transition.opacity.duration.150ms :style="coords" data-kebab-menu
                                        ...
                                        <form method="POST" action="{{ route('admin.blogs.categories.destroy', $category) }}"
    ...
    @foreach ($categories as $category)
        <x-admin-modal id="edit-category-{{ $category->id }}" title="Edit Category">
            <form method="POST" action="{{ route('admin.blogs.categories.update', $category) }}" class="space-y-4">
                @csrf @method('PUT')
    ...
    <x-admin-modal id="create-category-modal" title="Create Category">
        <form method="POST" action="{{ route('admin.blogs.categories.store') }}" class="space-y-4">
            @csrf

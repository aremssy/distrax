<x-admin-layout title="Hotel Bookings">
    @php
        $avatarStyles = [
    ...
    <x-admin-page-header title="Hotel Bookings"
        :breadcrumbs="[['label' => 'Hotel Bookings']]"
        description="Manage hotel/room bookings.">
        <x-slot name="actions">
            <x-admin.export-button resource="hotel-bookings" />
        </x-slot>
    </x-admin-page-header>

    {{-- Filter bar --}}
    <form method="GET" action="{{ route('admin.hotel-bookings.index') }}"
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Guest</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Check In</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Check Out</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800">
                    @forelse ($bookings as $booking)
                        @php
                            ...
                            $avatarStyle = $avatarStyles[crc32($guestName) % count($avatarStyles)];
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40">
                            ...
                            <td class="min-w-48 max-w-64 px-3 py-3.5">
                                <div class="min-w-0">
                                    ...
                                    <p class="mt-0.5 flex items-center gap-1 text-xs text-slate-400 dark:text-night-500">
                                        @include('admin.partials.icon', ['name' => 'key', 'class' => 'w-3.5 h-3.5 shrink-0 text-amber-500'])
                                        <span class="truncate">Booking #{{ $booking->id }}</span>
                                    </p>
                                </div>
                            </td>
                            ...
                            <td class="min-w-48 px-3 py-3.5">
                                <div class="flex items-center gap-3">
                                    <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold {{ $avatarStyle }}">
                                        {{ strtoupper(substr($guestName, 0, 2)) }}
                                    </span>
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">{{ $guestName }}</p>
                                        <p class="mt-0.5 flex items-center gap-1 text-xs text-slate-400 dark:text-night-500">
                            ...
                            <td class="whitespace-nowrap px-3 py-3.5">
                                <span class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ money($booking->amount) }}</span>
                            </td>

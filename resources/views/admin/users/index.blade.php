        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <select name="role_id" onchange="this.form.submit()"
                ...
                @foreach ($roles as $role)
                    <option value="{{ $role->id }}" @selected((int) request('role_id') === $role->id)>{{ $role->display_name }}</option>
                @endforeach
            </select>
            ...
            @if (request()->hasAny(['search', 'role_id', 'verification_status', 'is_blocked']))
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Verification</th>
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Joined</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800">
                    @forelse ($users as $user)
                        @php
                            ...
                            $verificationColor = match ($user->verification_status) {
                                'verified' => 'green',
                                'rejected' => 'red',
                                'pending' => 'yellow',
                                default => 'gray',
                        ...
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40 {{ $user->trashed() ? 'opacity-60' : '' }}">
                            ...
                            <td class="whitespace-nowrap px-3 py-3.5">
                                <x-admin-badge :color="$verificationColor">{{ ucfirst($user->verification_status ?? 'unverified') }}</x-admin-badge>
                            </td>

                            <td class="whitespace-nowrap px-3 py-3.5">
                                @if ($user->is_blocked)
                                    <x-admin-badge color="red">Blocked</x-admin-badge>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-16 text-center text-sm text-slate-400">No users found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

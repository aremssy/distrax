<x-admin-layout title="User Verification">
    ...
    <x-admin-page-header title="User Verification"
        :breadcrumbs="[['label' => 'Verification']]"
        description="Review and verify user identities.">
        <x-slot name="actions">
            <x-admin.export-button resource="verification" />
        </x-slot>
    </x-admin-page-header>
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Reviewed By</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Reviewed At</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800">
                    @forelse ($users as $user)
                        @php
                            $avatarStyle = $avatarStyles[crc32($user->name ?? '—') % count($avatarStyles)];
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40">
                            <td class="whitespace-nowrap px-5 py-3.5 font-mono text-xs text-slate-400">{{ $user->id }}</td>
                            ...
                            <td class="whitespace-nowrap px-3 py-3.5">
                                <x-admin-badge :color="match($user->verification_status) { 'pending' => 'yellow', 'verified' => 'green', 'rejected' => 'red', default => 'gray' }">
                                    {{ ucfirst($user->verification_status) }}
                                </x-admin-badge>
                            </td>

                            <td class="whitespace-nowrap px-3 py-3.5 text-sm text-slate-600 dark:text-slate-300">
                                {{ $user->verificationReviewer?->name ?? '—' }}
                            </td>

                            <td class="whitespace-nowrap px-3 py-3.5 text-sm text-slate-500 dark:text-slate-400">
                                {{ $user->verification_reviewed_at ? $user->verification_reviewed_at->format('M d, Y h:i A') : '—' }}
                            </td>
                                        ...
                                        <div x-cloak x-show="open" x-transition.opacity.duration.150ms :style="coords" data-kebab-menu
                                         class="fixed z-50 w-44 rounded-xl border border-slate-200 bg-white py-1 text-left shadow-xl dark:border-night-700 dark:bg-night-800">
                                            @if ($user->verification_document_path)
                                                <a href="{{ route('admin.verification.document', $user) }}" target="_blank" rel="noopener"
                                                   class="flex w-full items-center gap-2.5 px-3.5 py-2 text-sm text-slate-700 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-night-700/60">
                                                    @include('admin.partials.icon', ['name' => 'identification', 'class' => 'w-4 h-4'])
                                                    View document
                                                </a>
                                            @else
                                                <p class="px-3.5 py-2 text-xs text-slate-400">No document uploaded</p>
                                            @endif
                                            @if ($user->verification_status !== 'verified')
                                                <form method="POST" action="{{ route('admin.verification.update', $user) }}">
                                                    @csrf @method('PATCH')
                        ...
                        @can('users.approve')
                            <tr class="bg-transparent!">
                                <td colspan="6" class="px-5 pb-3.5 pt-0">
                                    <form method="POST" action="{{ route('admin.verification.update', $user) }}" class="flex items-center gap-2">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="status" value="{{ $user->verification_status === 'rejected' ? 'pending' : 'rejected' }}">
                                        <input type="text" name="note" placeholder="Add a note…"
                                               class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 placeholder-slate-400 focus:border-brand/50 focus:outline-none focus:ring-2 focus:ring-brand/10 dark:border-night-700 dark:bg-night-800 dark:text-slate-200 dark:placeholder-night-500">
                                        <button type="submit"
                                                class="shrink-0 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-xs transition-colors hover:bg-slate-50 dark:border-night-700 dark:bg-night-900 dark:text-slate-200 dark:hover:bg-night-800">
                                            Save Note
                                        </button>
                                    </form>
                                </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-16 text-center text-sm text-slate-400">No users found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

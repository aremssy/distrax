<x-admin-layout :title="'Edit: '.$role->display_name">
    <x-admin-page-header :title="'Edit: '.$role->display_name"
        :breadcrumbs="[['label' => 'Roles', 'route' => 'admin.roles.index'], ['label' => $role->display_name]]"
        description="Update role details and adjust permissions." />
            ...
            <x-admin-card title="Role Details">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <x-admin.form.input name="name" label="System Name" placeholder="e.g. content_manager"
                        :value="old('name', $role->name)" required hint="Lowercase, no spaces. Used internally." />

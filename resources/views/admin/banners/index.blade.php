<x-admin-layout title="Banners">
    <x-admin-page-header title="Banners"
        ...
        <x-slot name="actions">
            <x-admin.export-button resource="banners" />
            @can('banners.create')
                <button onclick="openModal('create-banner-modal')"
    ...
    <form method="GET" action="{{ route('admin.banners.index') }}"
        ...
        <div class="flex flex-wrap items-center gap-3">
            <select name="position" onchange="this.form.submit()"
                ...
                @foreach (['hero', 'sidebar', 'inline', 'popup', 'footer'] as $p)
                    <option value="{{ $p }}" @selected(request('position') === $p)>{{ ucfirst($p) }}</option>
                @endforeach
            </select>
    ...
    <div class="mb-8 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-night-700 dark:bg-night-900">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        ...
                        <th class="whitespace-nowrap px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Banner</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Position</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Starts</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Ends</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800"
                       data-sortable
                       data-sortable-url="{{ route('admin.banners.reorder') }}"
                       data-sortable-offset="{{ ($banners->currentPage() - 1) * $banners->perPage() }}">
                    @forelse ($banners as $banner)
                        @php
                            $imageUrl = $banner->image
                                ...
                                : null;
                        @endphp
                        <tr data-id="{{ $banner->id }}" class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40">
                            <td class="px-3 py-3.5 align-middle">@include('admin.partials.drag-handle')</td>
                            <td class="min-w-60 max-w-72 px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    @if ($imageUrl)
                                        <img src="{{ $imageUrl }}" alt="{{ $banner->name }}" loading="lazy"
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="mt-0.5 truncate font-mono text-xs text-slate-400 dark:text-night-500">{{ $banner->link ?? '—' }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-3 py-3.5 text-sm text-slate-500 dark:text-slate-400">
                                {{ $banner->starts_at?->format('M d, Y') ?? '—' }}
                            </td>

                            <td class="whitespace-nowrap px-3 py-3.5 text-sm text-slate-500 dark:text-slate-400">
                                {{ $banner->ends_at?->format('M d, Y') ?? '—' }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-16 text-center text-sm text-slate-400">No banners found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        {{ $banners->onEachSide(1)->links('admin.partials.pagination') }}
    </div>

    @can('banners.create')
        <x-admin-modal id="create-banner-modal" title="Create Banner">
            <form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <x-admin.form.input name="image" label="Image" type="file" accept="image/jpeg,image/png,image/webp,image/gif" hint="Maximum 2 MB." required />
                <x-admin.form.input name="link" label="Link URL" :value="old('link')" />
                ...
                <x-admin.form.input name="ends_at" label="Ends At" type="date" :value="old('ends_at')" />
                <x-admin.form.select name="is_active" label="Status">
                    <option value="1" @selected(old('is_active', true))>Active</option>
                    <option value="0">Inactive</option>
                </x-admin.form.select>
                <div class="flex justify-end gap-3 pt-2">
                    <button type="button" onclick="closeModal('create-banner-modal')" class="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-night-700 rounded-xl hover:bg-slate-50 dark:hover:bg-night-800 transition">Cancel</button>

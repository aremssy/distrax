<x-admin-layout title="Contact Messages">
    ...
    <x-admin-page-header title="Contact Messages"
        ...
        <x-slot name="actions">
            <x-admin.export-button resource="contact-messages" />
        </x-slot>
    </x-admin-page-header>
    ...
    <div class="mb-8 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-night-700 dark:bg-night-900">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">From</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Subject</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Message</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Received</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800">
                    @forelse ($messages as $message)
                        @php
                            $avatarStyle = $avatarStyles[crc32($message->name ?? '—') % count($avatarStyles)];
                            $isUnread = $message->status === 'new';
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40">
                            <td class="whitespace-nowrap px-5 py-3.5 font-mono text-xs text-slate-400 dark:text-night-500">{{ $message->id }}</td>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="flex items-center gap-1.5">
                                            ...
                                            @endif
                                            <span class="truncate text-sm font-semibold text-slate-800 dark:text-slate-100 {{ $isUnread ? 'font-bold' : '' }}">{{ $message->name }}</span>
                                        </p>
                            ...
                            <td class="px-3 py-3.5 text-sm text-slate-600 dark:text-slate-300">{{ $message->subject }}</td>

                            <td class="max-w-xs truncate px-3 py-3.5 text-xs text-slate-400 dark:text-night-500">{{ $message->message }}</td>

                            <td class="whitespace-nowrap px-3 py-3.5 text-sm text-slate-500 dark:text-slate-400">{{ $message->created_at->format('M d, Y') }}</td>

                            <td class="whitespace-nowrap px-3 py-3.5">
                                <x-admin-badge :color="match($message->status) { 'resolved' => 'green', 'read' => 'blue', default => 'indigo' }">
                                    {{ ucfirst($message->status) }}
                                </x-admin-badge>
                            </td>

                            <td class="whitespace-nowrap px-5 py-3.5 text-right">
                                @can('contact_messages.edit')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-16 text-center text-sm text-slate-400">No messages found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        {{ $messages->onEachSide(1)->links('admin.partials.pagination') }}
    </div>
</x-admin-layout>

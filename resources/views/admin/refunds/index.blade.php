<x-admin-layout title="Refunds">
    ...
    <x-admin-page-header title="Refunds"
        ...
        description="View and process refund requests.">
        <x-slot name="actions">
            <x-admin.export-button resource="refunds" />
        </x-slot>
    </x-admin-page-header>
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Payment</th>
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Requested By</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800">
                    @forelse ($refunds as $refund)
                        @php
                            $avatarStyle = $avatarStyles[crc32($refund->requestedBy?->name ?? '—') % count($avatarStyles)];
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40">
                            <td class="px-5 py-3.5 font-mono text-xs text-slate-400 dark:text-night-500">{{ $refund->id }}</td>
                            ...
                            <td class="whitespace-nowrap px-3 py-3.5">
                                <span class="text-sm font-bold text-slate-800 dark:text-slate-100">
                                    {{ $refund->payment ? money($refund->amount, $refund->payment->currency) : money($refund->amount) }}
                                </span>
                            </td>

                            <td class="max-w-64 px-3 py-3.5">
                                <p class="line-clamp-2 text-sm text-slate-600 dark:text-slate-300">{{ $refund->reason }}</p>
                            </td>

                            <td class="px-3 py-3.5">
                                <div class="flex items-center gap-2.5">
                                    <span class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold {{ $avatarStyle }}">
                                        {{ strtoupper(substr($refund->requestedBy?->name ?? '—', 0, 2)) }}
                                    </span>
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">{{ $refund->requestedBy?->name ?? '—' }}</p>
                                        <p class="truncate text-xs text-slate-400 dark:text-night-500">{{ $refund->requestedBy?->email }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-5 py-3.5 text-right">
                                    ...
                                    <div class="relative inline-block" x-data="kebabMenu()" @click.away="open = false">
                                        ...
                                        <div x-cloak x-show="open" x-transition.opacity.duration.150ms :style="coords" data-kebab-menu
                                         class="fixed z-50 w-44 rounded-xl border border-slate-200 bg-white py-1 text-left shadow-xl dark:border-night-700 dark:bg-night-800">
                                            ...
                                            <form method="POST" action="{{ route('admin.refunds.process', $refund) }}"
                                                  data-confirm="Reject this refund?">
                                                @csrf @method('PATCH')
                                ...
                                @else
                                    <span class="text-xs text-slate-400 dark:text-night-500">{{ ucfirst($refund->status) }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-16 text-center text-sm text-slate-400">No refunds found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>

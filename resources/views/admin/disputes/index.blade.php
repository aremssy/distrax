<x-admin-layout title="Disputes">
    ...
    <x-admin-page-header title="Disputes"
        :breadcrumbs="[['label' => 'Disputes']]"
        description="Manage and resolve user disputes.">
        <x-slot name="actions">
            <x-admin.export-button resource="disputes" />
        </x-slot>
    </x-admin-page-header>
    ...
    <form method="GET" action="{{ route('admin.disputes.index') }}"
        ...
        <div class="flex flex-wrap items-center gap-3">
            <div class="relative min-w-52 flex-1">
                <input type="search" name="search" value="{{ request('search') }}" placeholder="Search disputes..."
    ...
    <div class="mb-8 overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-night-700 dark:bg-night-900">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100 bg-slate-50/70 dark:border-night-800 dark:bg-night-800/50">
                        <th class="whitespace-nowrap px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Raised By</th>
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Against</th>
                        ...
                        <th class="whitespace-nowrap px-3 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-night-500">Assigned To</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-night-800">
                    @forelse ($disputes as $dispute)
                        @php
                            ...
                            $avatarStyle = $avatarStyles[crc32($raisedByName) % count($avatarStyles)];
                        @endphp
                        <tr class="transition-colors hover:bg-slate-50/60 dark:hover:bg-night-800/40">
                            <td class="min-w-60 max-w-72 px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        <p class="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">{{ $raisedByName }}</p>
                                        <p class="mt-0.5 truncate text-xs text-slate-400 dark:text-night-500">{{ $dispute->subject ?? '#'.$dispute->id }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-3 py-3.5">
                                <x-admin-badge :color="match($dispute->status) { 'open' => 'red', 'in_review' => 'yellow', 'resolved' => 'green', 'closed' => 'gray', default => 'gray' }">
                                                ...
                                                <form method="POST" action="{{ route('admin.disputes.update', $dispute) }}">
                                                    ...
                                                    <button type="submit"
                                                            @class([
                                                                ...
                                                                'text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-500/10' => $s === 'resolved',
                                                                'text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-500/10' => $s === 'closed',
                                                                'text-slate-600 hover:bg-slate-50 dark:text-slate-300 dark:hover:bg-night-700/60' => ! in_array($s, ['resolved', 'closed'], true),
                                                            ])>
                                                        @include('admin.partials.icon', [
                                                            'name' => $statusIcons[$s],
                                                            'class' => 'w-4 h-4'.(in_array($s, ['resolved', 'closed'], true) ? '' : ' text-slate-400'),
                                                        ])
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-16 text-center text-sm text-slate-400">No disputes found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        {{ $disputes->onEachSide(1)->links('admin.partials.pagination') }}
    </div>
</x-admin-layout>
